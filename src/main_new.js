...
class GameState {
  ...
  calculateStats() {
    ...
    this.player.attackSpeed = equipmentAttackSpeed;
    this.player.critChance = equipmentCritChance;
...
function updateUI() {
  ...
  for (const slot in game.player.equipment) {
    ...
    if (element) {
      element.textContent = item ? (item.fullName || item.name) : 'None';
      const slotElement = element.closest('.equipment-slot');
      if (item) {
        slotElement.classList.add('has-item');
        // Add rarity color
        if (item.rarity && ITEM_RARITIES[item.rarity]) {
          element.style.color = ITEM_RARITIES[item.rarity].color;
        }
      } else {
...
function updateInventory() {
  ...
  game.player.inventory.forEach((item, index) => {
    ...
    // Create stats display including new RPG stats
    let statsHtml = `ATK: ${item.attack || 0} | DEF: ${item.defense || 0}`;
    const additionalStats = [];
    
    if (item.attackSpeed > 0) additionalStats.push(`AS: ${item.attackSpeed}%`);
    if (item.critChance > 0) additionalStats.push(`CC: ${item.critChance}%`);
    if (item.critDamage > 0) additionalStats.push(`CD: ${item.critDamage}%`);
    if (item.lifeSteal > 0) additionalStats.push(`LS: ${item.lifeSteal}%`);
    if (item.dodge > 0) additionalStats.push(`DODGE: ${item.dodge}%`);
    if (item.blockChance > 0) additionalStats.push(`BLOCK: ${item.blockChance}%`);
    
    if (additionalStats.length > 0) {
      statsHtml += `<br>${additionalStats.join(' | ')}`;
    }
    ...
    itemDiv.innerHTML = `
      ...
      <div class="item-rarity" style="color: ${rarityColor}; font-size: 0.8em;">
        ${item.rarity ? ITEM_RARITIES[item.rarity].name : 'Common'}
      </div>
